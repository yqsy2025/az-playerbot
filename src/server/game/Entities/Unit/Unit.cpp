...
{
    // Calculate critical bonus//计算暴击伤害加成
    ...
    if (caster && (caster->IsPlayer() || (caster->IsPet() && caster->GetOwner()->ToPlayer())) && (spellProto->DmgClass == SPELL_DAMAGE_CLASS_MELEE || spellProto->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
    {
        crit_bonus *= 1.2f;  // 物理暴击伤害增加 20%
    }
    return crit_bonus;
}
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto, SpellSchoolMask damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    if (ToPlayer() || (ToPet() && ToPet()->GetOwner()->ToPlayer()))
        tmpDamage *= 1.2f; //提升所有物理伤害20%
