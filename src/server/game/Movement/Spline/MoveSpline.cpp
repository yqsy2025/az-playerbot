...
namespace Movement
{
    ...
    bool MoveSpline::isStairPath(const std::vector<Vector3>& path)
    {
        // 路径点数量不足，无法构成楼梯
        if (path.size() < 4)
            return false;

        constexpr float minStepHeight = 0.15f;  // 楼梯的最小单步高度
        constexpr float maxStepHeight = 0.5f;   // 楼梯的最大单步高度
        constexpr int minStairSteps = 3;        // 至少要有 3 级连续台阶才算楼梯
        constexpr int maxAllowedBigSteps = 1;   // 允许最多 1 个超出 maxStepHeight 的步长
        constexpr float minSlope = 0.1f;        // 最小斜率（高度差与水平距离的比值）
        constexpr float maxSlope = 1.0f;        // 最大斜率

        int stairStepCount = 0;                 // 符合楼梯特征的步数
        int bigStepCount = 0;                   // 超出最大步长的步数
        int consecutiveStairSteps = 0;          // 连续符合楼梯特征的步数

        // 检查路径点的高度变化方向是否一致（单调上升或单调下降）
        bool isAscending = (path[1].z > path[0].z);

        for (size_t i = 1; i < path.size(); ++i)
        {
            float heightDiff = path[i].z - path[i - 1].z;
            float horizontalDist = std::sqrt(std::pow(path[i].x - path[i - 1].x, 2) + std::pow(path[i].y - path[i - 1].y, 2));
            float slope = (horizontalDist > 0) ? std::abs(heightDiff) / horizontalDist : 0.0f;

            // 检查高度变化方向是否一致
            if ((isAscending && heightDiff < 0) || (!isAscending && heightDiff > 0))
            {
                return false; // 高度变化方向不一致，不是楼梯
            }

            // 符合楼梯步长范围且斜率在合理范围内
            if (std::abs(heightDiff) >= minStepHeight && std::abs(heightDiff) <= maxStepHeight &&
                slope >= minSlope && slope <= maxSlope)
            {
                ++stairStepCount;
                ++consecutiveStairSteps;

                // 如果连续符合楼梯特征的步数达到要求，则判定为楼梯
                if (consecutiveStairSteps >= minStairSteps)
                {
                    return true;
                }
            }
            else if (std::abs(heightDiff) > maxStepHeight)
            {
                ++bigStepCount;
                consecutiveStairSteps = 0; // 重置连续台阶计数

                // 如果超出范围的步长过多，则判定不是楼梯
                if (bigStepCount > maxAllowedBigSteps)
                {
                    return false;
                }
            }
    ...
    void MoveSpline::init_spline(const MoveSplineInitArgs& args)
    {
        ...
        // 检测路径是否是楼梯，如果是则强制使用平滑曲线
        SplineBase::EvaluationMode evalMode;
        if (isStairPath(args.path))
            evalMode = SplineBase::ModeCatmullrom;
        else
            evalMode = modes[args.flags.isSmooth()];
