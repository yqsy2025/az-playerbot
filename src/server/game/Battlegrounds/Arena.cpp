...
{
    uint8 duiwu = 0u; // 记录失败队伍
    if(winnerTeamId == TEAM_ALLIANCE)
        duiwu = TEAM_HORDE;
    else if (winnerTeamId == TEAM_HORDE)
        duiwu = TEAM_ALLIANCE;
    if (isRated() && GetPlayersCountByTeam(static_cast<TeamId>(duiwu)))
    {
        // 定义基础奖励的金币和物品数量
        const uint32 ITEM_ID = 38186; // 奖励的物品 ID
        const uint32 BASE_GOLD = 1;   // 基础金币奖励（每人 1G）
        ...
        {
        case ARENA_TYPE_1v1:
            goldAmount = BASE_GOLD * 1; // 1v1 队伍奖励 1G
            itemAmount = 1;             // 1v1 队伍奖励 1 个虚灵币
            break;
        case ARENA_TYPE_2v2:
            goldAmount = BASE_GOLD * 2; // 2v2 队伍奖励 2G
            itemAmount = 1;             // 2v2 队伍奖励 1 个虚灵币
            break;
        case ARENA_TYPE_3v3:
            goldAmount = BASE_GOLD * 3; // 3v3 队伍奖励 3G
            itemAmount = 1;             // 3v3 队伍奖励 1 个虚灵币
            break;
        case ARENA_TYPE_5v5:
            goldAmount = BASE_GOLD * 5; // 5v5 队伍奖励 5G
            itemAmount = 2;             // 5v5 队伍奖励 2 个虚灵币
            break;
        default:
            goldAmount = 0; // 默认无奖励
            itemAmount = 0;
        ...
        {
            // 检查输家是否有指定的物品且不是bot
            bool shouldRewardWinner = true;
            for (auto const& [playerGuid, player] : GetPlayers())
            {
                if (player->GetBgTeamId() != winnerTeamId)
                {
                    if (player->GetItemCount(ITEM_ID, 1) < itemAmount && !player->GetSession()->IsBot())
                    {
                        shouldRewardWinner = false;
                        break;
                    }
                }
            }
            ...
            if (shouldRewardWinner)
            {
                for (auto const& [playerGuid, player] : GetPlayers())
                {
                    ...
                    {
                        // 添加物品
                        player->AddItem(ITEM_ID, itemAmount); // 奖励物品
                        // 添加金币
                        player->ModifyMoney(goldAmount * GOLD); // 奖励金币
                        // 发送系统消息通知玩家
                        std::string rewardMessage = "获胜奖励 " + std::to_string(goldAmount) + " 金币和 " + std::to_string(itemAmount) + " 个虚灵币";
                        player->SendSystemMessage(rewardMessage, true);
                    }
                }
            }
            // 惩罚失败方
            for (auto const& [playerGuid, player] : GetPlayers())
            {
                ...
                {
                    // 检查玩家是否有足够的物品
                    if (player->GetItemCount(ITEM_ID, 1) >= itemAmount)
                    {
                        // 移除物品
                        player->DestroyItemCount(ITEM_ID, itemAmount, true); // 移除物品
                    }

                    // 检查玩家是否有足够的金币
                    if (player->GetMoney() >= goldAmount * GOLD)
                    {
                        // 扣除金币
                        player->ModifyMoney(-int32(goldAmount * GOLD)); // 扣除金币
                    }
                    // 发送系统消息通知玩家
                    std::string lostMessage = "输掉了 " + std::to_string(goldAmount) + " 金币和 " + std::to_string(itemAmount) + " 个虚灵币";
                    player->SendSystemMessage(lostMessage, true);
                }
            }
        }
